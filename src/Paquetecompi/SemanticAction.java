    ...
    void execute(Lexer lex,StringBuilder lexeme, char currentChar) {
        ...
        if (lex.isReservedWord(token)) {
            ...
            if (!lex.containsSymbol(token)) {
                lex.insertSymbolTable(token, "String",null,null, SymbolTable.identifierValue);
            }
            lex.setDevolvi(true);
    ...
    void execute(Lexer lex,StringBuilder lexeme, char currentChar) {
        ...
        if (token.startsWith("0") && !token.matches(".*[89].*")) { //si el numero es octal (empieza con 0 y no contiene ni 8 ni 9
            try {
                ...
                lex.insertSymbolTable(representacionOctal, "Octal","Constante",null, SymbolTable.constantValue);
                lex.setDevolvi(true);
            ...
            try {
                ...
                lex.insertSymbolTable(token, "longint","Constante",null, SymbolTable.constantValue);
                lex.setDevolvi(true);
                lex.addToken(new Pair(lexeme.toString(), SymbolTable.constantValue));
            } catch (NumberFormatException e) {
    ...
    void execute(Lexer lex, StringBuilder lexeme, char currentChar) {
        ...
        try {
            ...
            if ((number.compareTo(minPositive) > 0 && number.compareTo(maxPositive) < 0) || 
                ...
                lex.insertSymbolTable(lexeme.toString(), "double","Constante",null, SymbolTable.constantValue);
                lex.setDevolvi(true);
                lex.addToken(new Pair(lexeme.toString(), SymbolTable.constantValue)); 
            } else {
                ...
                lex.insertSymbolTable(token, "double","Constante",null, SymbolTable.constantValue);
                lex.setDevolvi(true);
                lex.addToken(new Pair(lexeme.toString(), SymbolTable.constantValue)); 
            }
        } catch (NumberFormatException e) {
...
class AS7 extends SemanticAction {//TABLA PALABRA RESERVADAS
    ...
    void execute(Lexer lex,StringBuilder lexeme, char currentChar) {
		...
		if (currentChar == ']') {
		    ...
		    lex.insertSymbolTable(cadenaSinSaltos, "String","Cadena multilinea",null, SymbolTable.stringValue);
		    valor = SymbolTable.stringValue;
		}else if(currentChar == '@') {
    		lex.insertSymbolTable(lexeme.toString(),"String","Nombre de etiqueta",null,SymbolTable.tagValue);
